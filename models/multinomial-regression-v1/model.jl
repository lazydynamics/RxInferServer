...
function create_transformation(transformation_name::String)
    if !haskey(TRANSFORMATION_DISPATCH_TABLE, transformation_name)
        throw(ArgumentError("Invalid transformation name: $transformation_name. Available options: $(keys(TRANSFORMATION_DISPATCH_TABLE))"))
    end
    ...
    for i in eachindex(obs)
        ...
        obs[i] ~ MultinomialPolya(N, Ψ[i]) where {
            dependencies = RequireMessageFunctionalDependencies(
                ψ = MvNormalWeightedMeanPrecision(zeros(k), diageye(k))
            )
        }
    end
end
...
function run_inference(state, parameters, data)
    ...
    inference_results = infer(
        model = multinomial_regression_predictive(
            X = X, 
            N = N, 
            ϕ = ϕ, 
            ξβ = ξβ, 
...
function run_learning(state, parameters, events)
    ...
    if !isempty(X) && !isempty(obs)
        ...
        for (i, o) in enumerate(obs)
            if length(o) != K
                throw(DimensionMismatch("All observations must have the same number of categories. Expected $K, got $(length(o)) at index $i"))
            end
            if sum(o) != N
        ...
        parameters = Dict(
            ...
            "beta_precision" => precision(β_posterior)
        )
    end
    
    result = Dict(
        "beta_mean" => parameters["beta_mean"],
        "beta_precision" => parameters["beta_precision"]
    )
