...
function get_model_info_invoke(impl; post_invoke=nothing)
    function get_model_info_invoke_handler(req::HTTP.Request)
        openapi_params = req.context[:openapi_params]
        ret = impl.get_model_info(req::HTTP.Request, openapi_params["model_name"];)
...
function registerModelsApi(router::HTTP.Router, impl; path_prefix::String="", optional_middlewares...)
    HTTP.register!(router, "GET", path_prefix * "/models/{model_name}/info", OpenAPI.Servers.middleware(impl, get_model_info_read, get_model_info_validate, get_model_info_invoke; optional_middlewares...))
