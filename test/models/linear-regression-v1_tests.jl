...
@testitem "it should be possible to create a model instance" setup = [TestUtils] begin
    ...
    create_model_instance_request = TestUtils.RxInferClientOpenAPI.CreateModelInstanceRequest(
        model_name = "LinearRegression-v1",
        description = "Testing linear regression model"
    )
...
@testitem "model should be able to learn from data and make predictions" setup = [TestUtils] begin
    ...
    @testset for n in (1000, 2000), true_a in (0.5, 2.0, -1.0), true_b in (0.0, 5.0, -3.0), true_noise in (0.1, 0.5, 2.0)
        ...
        # Check that inference results contain the expected parameters
        ...
        @test all(isapprox.(inference_response.results["y_mean"], test_y_true, atol = 0.1))
